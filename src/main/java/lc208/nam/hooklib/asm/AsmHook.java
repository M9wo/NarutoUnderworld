...
public class AsmHook implements Cloneable, Comparable<AsmHook> {
  ...
  protected boolean isTargetMethod(String name, String desc) {
    return (((this.targetMethodReturnType == null && desc.startsWith(this.targetMethodDescription)) || desc
      .equals(this.targetMethodDescription)) && name.equals(this.targetMethodName));
  }
  ...
  protected void createMethod(HookInjectorClassVisitor classVisitor) {
    ClassMetadataReader.MethodReference superMethod = classVisitor.transformer.classMetadataReader.findVirtualMethod(getTargetClassInternalName(), this.targetMethodName, this.targetMethodDescription);
    ...
    if (mv instanceof HookInjectorMethodVisitor) {
      ...
      if (superMethod == null) {
        injectDefaultValue(inj, this.targetMethodReturnType);
      } else {
        injectSuperCall(inj, superMethod);
      } 
      injectReturn(inj, this.targetMethodReturnType);
      inj.visitLabel(new Label());
      inj.visitMaxs(0, 0);
      inj.visitEnd();
    } else {
  ...
  protected void inject(HookInjectorMethodVisitor inj) {
    ...
    if (hasHookMethod()) {
      injectInvokeStatic(inj, returnLocalId, this.hookMethodName, this.hookMethodDescription);
    ...
    if (this.returnCondition != ReturnCondition.NEVER) {
      ...
      if (this.returnValue == ReturnValue.NULL) {
        ...
        injectInvokeStatic(inj, returnLocalId, this.returnMethodName, returnMethodDescription);
      } 
  ...
  private void injectInvokeStatic(HookInjectorMethodVisitor inj, int returnLocalId, String name, String desc) {
    ...
    while (true) {
      int variableId;
      if (i < this.hookMethodParameters.size()) {
        Type parameterType = this.hookMethodParameters.get(i);
        variableId = ((Integer)this.transmittableVariableIds.get(i)).intValue();
        if (inj.isStatic) {
          if (variableId == 0) {
            inj.visitInsn(1);
          } else {
    ...
    inj.visitMethodInsn(184, getHookClassInternalName(), name, desc, false);
  }
  ...
  public class Builder extends AsmHook {
    ...
    public Builder addHookMethodParameter(Type parameterType, int variableId) {
      ...
      AsmHook.this.transmittableVariableIds.add(Integer.valueOf(variableId));
      return this;
    }
    ...
    public Builder addThisToHookMethodParameters() {
      ...
      AsmHook.this.transmittableVariableIds.add(Integer.valueOf(0));
      return this;
    }
    ...
    public Builder addReturnValueToHookMethodParameters() {
      ...
      AsmHook.this.transmittableVariableIds.add(Integer.valueOf(-1));
    ...
    private String getMethodDesc(Type returnType, List<Type> paramTypes) {
      Type[] paramTypesArray = paramTypes.<Type>toArray(new Type[0]);
      if (returnType == null) {
    ...
    public AsmHook build() {
      ...
      if (hook.hasHookMethod())
        hook.hookMethodDescription = Type.getMethodDescriptor(hook.hookMethodReturnType, 
            (Type[])hook.hookMethodParameters.toArray((Object[])new Type[0])); 
