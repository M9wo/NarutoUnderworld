...
public class RenderMistServant extends RenderBiped {
  private static final ResourceLocation Player_Texture = new ResourceLocation("mtm", "textures/entity/steve.png");
  
  Minecraft mc = Minecraft.getMinecraft();
  
  public RenderMistServant(ModelBiped p_i1262_1_, float p_i1262_2_) {
    super(p_i1262_1_, p_i1262_2_);
  }
  
  public RenderMistServant(ModelBunshin model, float size) {
    super(model, size);
  }
  
  protected ResourceLocation getEntityTexture(Entity entity) {
    EntityMistServant clone = (EntityMistServant)entity;
    if (clone != null) {
