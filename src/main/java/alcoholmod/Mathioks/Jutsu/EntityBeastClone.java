...
public class EntityBeastClone extends EntityInuzuka {
  public int kind = 0;
  ...
  public EntityBeastClone(World par1World) {
    super(par1World);
  ...
  public boolean attackEntityAsMob(Entity par1Entity) {
    ...
    if (getOwner() != null) {
      if (this.skin == 1) {
        ...
        return par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), (float)(5.0D + extendedPlayer.getTaijutsu() / 4.0D));
      } 
      if (this.skin == 2) {
        ExtendedPlayer extendedPlayer = ExtendedPlayer.get(owner);
        return par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), (float)(7.0D + extendedPlayer.getTaijutsu() / 4.0D));
      } 
      if (this.skin == 3) {
        ExtendedPlayer extendedPlayer = ExtendedPlayer.get(owner);
        return par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), (float)(10.0D + extendedPlayer.getTaijutsu() / 4.0D));
      } 
      if (this.skin == 4) {
        ExtendedPlayer extendedPlayer = ExtendedPlayer.get(owner);
        return par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), (float)(13.0D + extendedPlayer.getTaijutsu() / 4.0D));
      } 
      ExtendedPlayer props = ExtendedPlayer.get(owner);
      return par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), (float)(5.0D + props.getTaijutsu() / 4.0D));
    } 
  ...
  public void onUpdate() {
    ...
    if (this.ticksExisted >= 1800) {
      if (this.kind == 0)
        setDead(); 
      if (getOwner() != null && (getOwner()).worldObj != null) {
        if (this.kind == 1) {
          InuzukaEntityAkamaru fireball2 = new InuzukaEntityAkamaru((getOwner()).worldObj);
          fireball2.setPosition(this.posX, this.posY, this.posZ);
          fireball2.func_152115_b(getOwner().getUniqueID().toString());
          fireball2.setTamed(true);
          fireball2.setCustomNameTag(this.name);
          (getOwner()).worldObj.spawnEntityInWorld((Entity)fireball2);
          EntityPlayer Entity = (EntityPlayer)getOwner();
          Entity.addChatComponentMessage((IChatComponent)new ChatComponentText(this.name + " has transformed back to dog form!"));
          setDead();
        } 
        if (this.kind == 2) {
          InuzukaEntityKuromaru fireball2 = new InuzukaEntityKuromaru((getOwner()).worldObj);
          fireball2.setPosition(this.posX, this.posY, this.posZ);
          fireball2.func_152115_b(getOwner().getUniqueID().toString());
          fireball2.setTamed(true);
          fireball2.setCustomNameTag(this.name);
  ...
  protected void onDeathUpdate() {
    if (!this.worldObj.isRemote && 
      getOwner() != null) {
      EntityPlayer Entity = (EntityPlayer)getOwner();
      Entity.addChatComponentMessage((IChatComponent)new ChatComponentText(this.name + " is hurt and decided to rest it off"));
    } 
    this.deathTicks++;
