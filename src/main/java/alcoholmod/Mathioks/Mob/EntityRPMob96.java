...
public class EntityRPMob96 extends EntityTameable {
  ...
  public EntityRPMob96(World p_i1696_1_) {
    super(p_i1696_1_);
    setSize(0.6F, 2.1F);
  ...
  public void onUpdate() {
    ...
    if (getOwner() != null) {
      ...
      if (this.TalkInt == 10) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : I have no intention to join you!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 11) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : I have no reason to tell you anything!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 12) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : I've got some things to take care of!!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 13) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : You will turn out to be a fine shinobi"));
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : But never forget that those who leave their friends"));
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : over the mission are the real scum!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 14) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : You seem like a person worth having as a friend"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 1) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Lightning Style: Lightning Cutter!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 2) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Water Style: Water Dragon Jutsu!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 3) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Fire Style: Great Fireball Jutsu!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 4) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : White Fang!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 19) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : This is why they call me Kakashi of the 1000 Techniques"));
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Sharingan!!"));
        this.TalkInt = 0;
      } 
      if (this.TalkInt == 30) {
        getOwner().addPotionEffect(new PotionEffect(Potion.damageBoost.id, 500, 1));
        getOwner().addPotionEffect(new PotionEffect(Potion.moveSpeed.id, 500, 1));
        getOwner().addPotionEffect(new PotionEffect(Potion.digSpeed.id, 500, 1));
        getOwner().addPotionEffect(new PotionEffect(Potion.resistance.id, 500, 0));
        this.TalkInt = 0;
      } 
      if (isSitting() && this.SittingTalk)
        if (!this.Invul) {
          Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Lead the way"));
          this.SittingTalk = false;
        } else {
          Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Urghh.. Give me a few more minutes!"));
          this.SittingTalk = false;
        }  
      if (!isSitting() && this.SittingTalk) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : I'll be here when you need me"));
        this.SittingTalk = false;
      } 
      if (this.FirstTamed) {
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Alright " + getAttackTarget().getCommandSenderName() + " I'll join you!"));
      ...
      if (getAttackTarget() != null) {
        ...
        if (randomNumberSay == 1) {
          ...
          if (randomNumber == 0)
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Hey " + getAttackTarget().getCommandSenderName() + " I'm your opponent!")); 
          if (randomNumber == 1)
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : " + getAttackTarget().getCommandSenderName() + " Let me show you why they call me Kakashi of the 1000 techniques!")); 
          if (randomNumber == 2)
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Hey " + getAttackTarget().getCommandSenderName() + " Over here!")); 
          if (randomNumber == 3)
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Stay back, I'll handle this!")); 
          if (randomNumber == 4)
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : I won't lose!")); 
          if (randomNumber == 5) {
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : The hole in one's heart gets filled by others around you"));
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : " + getAttackTarget().getCommandSenderName() + " I won't let you take that from me!"));
          } 
          if (randomNumber == 6)
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : " + getAttackTarget().getCommandSenderName() + ", Those who abandon their friends are worse than scum!")); 
          if (randomNumber == 7) {
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : " + getOwner().getCommandSenderName() + " Don't worry, I'll protect you guys!"));
            Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : even if it kills me, I don't let... My comrades die"));
          } 
        } 
      } 
    } 
  ...
  public boolean attackEntityAsMob(Entity par1Entity) {
    ...
    if (randomNumber >= 1499 && randomNumber <= 1530) {
      ...
      ((EntityLivingBase)par1Entity).addPotionEffect(new PotionEffect(Potion.confusion.id, 100, 4));
    ...
    if (randomNumber >= 100 && randomNumber <= 125) {
      WaterDragonEntity entitysnowball = new WaterDragonEntity(this.worldObj, (EntityLivingBase)this);
      ...
      addPotionEffect(new PotionEffect(Potion.fireResistance.id, 25, 2));
      this.TalkInt = 2;
    } 
  ...
  protected void onDeathUpdate() {
    if (getOwner() != null && !this.worldObj.isRemote) {
      ...
      if (randomNumber == 1)
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Urghhh!! " + getOwner().getCommandSenderName() + " Can you hold them off for a bit?")); 
      if (randomNumber == 2)
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Obito.. Your past will... Is still... close to me...!")); 
      if (randomNumber == 3)
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : I... Refuse... To let my comrades... Die....")); 
      if (randomNumber == 0)
        Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Hey " + getOwner().getCommandSenderName() + " I can't go on for now...")); 
      Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : " + getOwner().getCommandSenderName() + " I'll use my last powers to give you a little boost.."));
      Entity.addChatComponentMessage((IChatComponent)new ChatComponentText("Kakashi : Sharingan!!"));
    } 
    setHealth(10.0F);
    this.Invul = true;
