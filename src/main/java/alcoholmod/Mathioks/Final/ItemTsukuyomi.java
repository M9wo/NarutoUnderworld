...
public class ItemTsukuyomi extends JutsuItem {
  @SideOnly(Side.CLIENT)
  public EnumRarity getRarity(ItemStack par1ItemStack) {
    return EnumRarity.epic;
  }
  
  public ItemTsukuyomi() {
    this.maxStackSize = 1;
  }
      ...
      if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
        ...
        if (props.getSharinganActive()) {
          if (props.getSharingan() >= 4) {
            if (props.getCurrentMana() >= 200 + props.getNinjutsu() / 600) {
              Par2World.spawnEntityInWorld((Entity)new EntityTsukuyomi(Par2World, (EntityLivingBase)Par3EntityPlayer));
              Par3EntityPlayer.addChatComponentMessage((IChatComponent)new ChatComponentText("Tsukuyomi!!"));
              Par1ItemStack.stackTagCompound.setInteger("counter", 400);
              props.consumeMana(200 + props.getNinjutsu() / 600);
            } else {
  ...
  public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    ...
    par3List.add(EnumChatFormatting.DARK_AQUA + "Puts the target in an extremely strong genjutsu");
    par3List.add(EnumChatFormatting.GOLD + "Chakra Cost: " + (200 + props.getNinjutsu() / 600));
