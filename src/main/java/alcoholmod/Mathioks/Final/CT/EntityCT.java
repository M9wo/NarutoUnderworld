...
public class EntityCT extends EntityThrowable {
  ...
  public EntityCT(World par1World) {
    super(par1World);
  }
  
  public EntityCT(World par1World, EntityLivingBase par2Entity, double par3, double par4, double par5) {
  ...
  public void onUpdate() {
    ...
    super.onUpdate();
    this.counter++;
    if (!this.worldObj.isRemote && this.counter >= 100 && this.counter <= 850)
      this.size += 2; 
    if ((!this.worldObj.isRemote && this.thrower == null) || this.ticksExisted >= 2800)
      setDead(); 
    if (this.canFly) {
      this.motionX += 0.003D * Math.sin(-this.thrower.rotationYaw / 180.0D * Math.PI);
      this.motionY += 0.013D;
      this.motionZ += 0.003D * Math.cos(-this.thrower.rotationYaw / 180.0D * Math.PI);
      moveEntity(this.motionX, this.motionY, this.motionZ);
      if (this.posY - this.thrower.posY >= this.dis - 10.0D)
        this.canFly = false; 
    } else {
      this.motionX = 0.0D;
      this.motionY = 0.0D;
      this.motionZ = 0.0D;
      List<Entity> list = this.worldObj.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(this.posX - this.dis, this.posY - this.dis, this.posZ - this.dis, this.posX + this.dis, this.posY + this.dis, this.posZ + this.dis));
      for (int k2 = 0; k2 < list.size(); k2++) {
        Entity entity = list.get(k2);
        if (!(entity instanceof EntityCT) && !(entity instanceof SixPathsCTEntity) && !(entity instanceof RinneganUnivPullEntity) && getThrower() != null) {
          ...
          if (entity.getDistance(this.posX, this.posY, this.posZ) <= r * 2.3D && entity instanceof net.minecraft.entity.item.EntityFallingBlock) {
            if (!this.worldObj.isRemote || this.ticksExisted >= 2800)
              entity.setDead(); 
            this.size++;
          } 
        } 
      ...
      if (this.size > 0) {
        ...
        double r = Math.cbrt(this.size) * 5.0D;
        ...
        for (int i = 0; i <= MathHelper.floor_double(r); i++) {
          ...
          if (!this.worldObj.isRemote) {
            this.worldObj.setBlock(x, y, z, Blocks.dirt, 0, 2);
            this.worldObj.setBlock(x1, y1, z1, Blocks.dirt, 0, 2);
            this.worldObj.setBlock(x2, y2, z2, Blocks.dirt, 0, 2);
          } 
        } 
      } 
    } 
