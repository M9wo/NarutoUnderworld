...
public class FlameDragonItem extends JutsuItem {
  @SideOnly(Side.CLIENT)
  public EnumRarity getRarity(ItemStack par1ItemStack) {
    return EnumRarity.epic;
  }
  
  public FlameDragonItem() {
    this.maxStackSize = 1;
  }
    ...
    if (!Par2World.isRemote)
      if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
        ...
        if (props.getFireRelease() == 1)
          if (props.getCurrentMana() >= 50 + props.getNinjutsu() / 300) {
            if (!Par2World.isRemote) {
              int d = MathHelper.floor_double((Par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 0x3;
              Vec3 look = Par3EntityPlayer.getLookVec();
              FlameDragonEntity fireball2 = new FlameDragonEntity(Par2World, (EntityLivingBase)Par3EntityPlayer);
              ...
              props.consumeMana(50 + props.getNinjutsu() / 300);
            } 
          } else {
  ...
  public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    ...
    par3List.add(EnumChatFormatting.GOLD + "Chakra Cost: " + (50 + props.getNinjutsu() / 300));
