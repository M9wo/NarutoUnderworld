...
public class FireReleaseFireBulletJutsu extends JutsuItem {
  @SideOnly(Side.CLIENT)
  public EnumRarity getRarity(ItemStack par1ItemStack) {
    return EnumRarity.epic;
  }
  
  public FireReleaseFireBulletJutsu() {
    setMaxDamage(200);
    isDamageable();
    this.maxStackSize = 1;
  }
        ...
        if (props.getCurrentMana() >= 35 + props.getNinjutsu() / 600)
          if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
            if (Par1ItemStack.stackTagCompound.getInteger("counter") == 0) {
              Par2World.spawnEntityInWorld((Entity)new EntityFireBulletJutsu(Par2World, (EntityLivingBase)Par3EntityPlayer));
              props.consumeMana(35 + props.getNinjutsu() / 600);
              Par1ItemStack.stackTagCompound.setInteger("counter", 64);
            } 
          } else {
