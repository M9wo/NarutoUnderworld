...
public class BoilReleaseMeltdown extends JutsuItem {
  ...
  public ItemStack onItemRightClick(ItemStack Par1ItemStack, World Par2World, EntityPlayer Par3EntityPlayer) {
    if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
      if (!Par2World.isRemote) {
        ...
        if (props.getboilRelease() == 1 && Par1ItemStack.stackTagCompound.getInteger("counter") == 0)
          if (props.getCurrentMana() >= 35 + props.getNinjutsu() / 600) {
            Par2World.spawnEntityInWorld((Entity)new BoilReleaseMeltdownEntity(Par2World, (EntityLivingBase)Par3EntityPlayer));
            props.consumeMana(35 + props.getNinjutsu() / 600);
            Par1ItemStack.stackTagCompound.setInteger("counter", 42);
            Par3EntityPlayer.addChatComponentMessage((IChatComponent)new ChatComponentText("Boil Release: Meltdown!"));
          } else {
