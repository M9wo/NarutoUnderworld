...
public class LightningReleaseStaticForce extends JutsuItem {
  @SideOnly(Side.CLIENT)
  public EnumRarity getRarity(ItemStack par1ItemStack) {
    return EnumRarity.epic;
  }
  
  public LightningReleaseStaticForce() {
    this.maxStackSize = 1;
  }
  ...
  public void onUpdate(ItemStack par1ItemStack, World par2World, Entity par3Entity, int par4, boolean par5) {
    ...
    if (MistMode) {
      ...
      if (((EntityPlayer)par3Entity).isInWater() || ((EntityPlayer)par3Entity).worldObj.getBlock(MathHelper.floor_double(((EntityPlayer)par3Entity).posX), MathHelper.floor_double(((EntityPlayer)par3Entity).posY - 0.2D - ((EntityPlayer)par3Entity).yOffset), MathHelper.floor_double(((EntityPlayer)par3Entity).posZ)) == Blocks.water)
        list = par2World.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(((EntityPlayer)par3Entity).posX - 20.0D, ((EntityPlayer)par3Entity).posY - 3.0D, ((EntityPlayer)par3Entity).posZ - 20.0D, ((EntityPlayer)par3Entity).posX + 20.0D, ((EntityPlayer)par3Entity).posY + 3.0D, ((EntityPlayer)par3Entity).posZ + 20.0D)); 
        ...
        for (int k2 = 0; k2 < list.size(); k2++) {
          if (!par2World.isRemote) {
            ...
            if (entity instanceof net.minecraft.entity.EntityLiving && entity instanceof EntityLivingBase)
              ((EntityLivingBase)entity).addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 20, 6)); 
        ...
        if (props.getLightningRelease() == 1) {
          if (props.getCurrentMana() >= 5 + props.getNinjutsu() / 600) {
            ...
            if (counter == 0) {
              ...
              Par3EntityPlayer.addChatComponentMessage((IChatComponent)new ChatComponentText("Lightning Release: Static Force!"));
            } 
          } 
        } else {
  ...
  public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    ...
    par3List.add(EnumChatFormatting.DARK_AQUA + "A lightning release jutsu that creates");
    par3List.add(EnumChatFormatting.DARK_AQUA + "static electricity to paralyze anyone");
    par3List.add(EnumChatFormatting.DARK_AQUA + "close to the user for a short amount of time");
    par3List.add(EnumChatFormatting.RED + "Chakra Cost: 5");
  }
}
