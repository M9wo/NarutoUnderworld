...
public class YinReleaseSwordsOfDarkness extends JutsuItem {
  @SideOnly(Side.CLIENT)
  public EnumRarity getRarity(ItemStack par1ItemStack) {
    return EnumRarity.epic;
  }
  
  public YinReleaseSwordsOfDarkness() {
    this.maxStackSize = 1;
  }
    ...
    if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
      if (!Par2World.isRemote && Par1ItemStack.stackTagCompound.getInteger("kd") == 0) {
        ...
        if (props.getCurrentMana() >= 30 + props.getNinjutsu() / 600) {
          Par2World.spawnEntityInWorld((Entity)new YinReleaseSwordsEntity(Par2World, (EntityLivingBase)Par3EntityPlayer));
          props.consumeMana(30 + props.getNinjutsu() / 600);
          Par1ItemStack.stackTagCompound.setInteger("kd", 14);
          Par3EntityPlayer.addChatComponentMessage((IChatComponent)new ChatComponentText("Yin Release: Sword of Darkness"));
        } else {
  ...
  public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    ...
    par3List.add(EnumChatFormatting.DARK_AQUA + "A Yin Release jutsu that shoots out");
    par3List.add(EnumChatFormatting.DARK_AQUA + "a dark chakra blade");
