...
public class FireReleasePhoenixFlowerJutsu extends JutsuItem {
  ...
  public void onUpdate(ItemStack par1ItemStack, World Par2World, Entity Par3EntityPlayer, int par4, boolean par5) {
    ...
    if (FireMode) {
      ...
      if (counter == 5)
        Par2World.spawnEntityInWorld((Entity)new EntityPhoenixFlowerJutsu(Par2World, (EntityLivingBase)Par3EntityPlayer)); 
      if (counter == 15)
        Par2World.spawnEntityInWorld((Entity)new EntityPhoenixFlowerJutsu(Par2World, (EntityLivingBase)Par3EntityPlayer)); 
      if (counter == 25)
        Par2World.spawnEntityInWorld((Entity)new EntityPhoenixFlowerJutsu(Par2World, (EntityLivingBase)Par3EntityPlayer)); 
  ...
  public FireReleasePhoenixFlowerJutsu() {
    setMaxDamage(3700);
    this.maxStackSize = 1;
  }
  ...
  public ItemStack onItemRightClick(ItemStack Par1ItemStack, World Par2World, EntityPlayer Par3EntityPlayer) {
    if (!Par2World.isRemote) {
      ...
      if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
        boolean FireMode = Par1ItemStack.stackTagCompound.getBoolean("FireMode");
        int counter = Par1ItemStack.stackTagCompound.getInteger("counter");
        if (props.getCurrentMana() >= 15.0D + 0.015D * props.getLevel() && !FireMode) {
