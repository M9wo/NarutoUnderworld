...
public class SummonTurtle extends Item {
  ...
  public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
    if (par1ItemStack.getItemDamage() <= 0) {
      EntityTurtle entityturtle = new EntityTurtle(par3World);
      if (!par3World.isRemote) {
        entityturtle.func_152115_b(par2EntityPlayer.getUniqueID().toString());
        entityturtle.setTamed(true);
        entityturtle.setLocationAndAngles(par4 + 0.5D, par5 + 0.5D, par6 + 0.5D, MathHelper.wrapAngleTo180_float(par3World.rand.nextFloat() * 360.0F), 0.0F);
        entityturtle.rotationYawHead = entityturtle.rotationYaw;
        entityturtle.renderYawOffset = entityturtle.rotationYaw;
        par3World.spawnEntityInWorld((Entity)entityturtle);
