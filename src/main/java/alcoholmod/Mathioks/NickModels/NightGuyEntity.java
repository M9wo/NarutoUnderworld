...
public class NightGuyEntity extends EntityTameable {
  ...
  public NightGuyEntity(World par1World) {
    super(par1World);
  ...
  public void onUpdate() {
    ...
    if (this.ticksExisted >= 200)
      setDead(); 
    List<Entity> list = this.worldObj.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(this.posX - 5.0D, this.posY - 3.0D, this.posZ - 5.0D, this.posX + 5.0D, this.posY + 3.0D, this.posZ + 5.0D));
    if (this != null)
      for (int k2 = 0; k2 < list.size(); k2++) {
        if (!this.worldObj.isRemote) {
          ...
          if (entity instanceof net.minecraft.entity.EntityLiving && entity instanceof EntityLivingBase && entity != this)
            entity.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer)getOwner()), 5.0F); 
          if (entity instanceof EntityPlayer && !entity.getUniqueID().toString().equals(getOwner().getUniqueID().toString()))
            entity.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer)getOwner()), 5.0F); 
        } 
      }  
  }
