...
public class SyncBijuuSlashMessage extends AbstractMessage {
  private boolean data;
  
  public SyncBijuuSlashMessage() {}
  
  public SyncBijuuSlashMessage(boolean data, EntityPlayer player) {
    this.data = data;
  }
  ...
  public void process(EntityPlayer player, Side side) {
    ...
    List<Entity> list = player.worldObj.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(player.posX - 10.0D, player.posY - 20.0D, player.posZ - 10.0D, player.posX + 20.0D, player.posY + 20.0D, player.posZ + 20.0D));
    if (player != null) {
      ...
      player.addChatMessage((IChatComponent)new ChatComponentText("Bijuu Slash!"));
    } 
  }
