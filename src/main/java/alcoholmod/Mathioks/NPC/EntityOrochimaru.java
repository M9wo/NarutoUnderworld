...
public class EntityOrochimaru extends EntitySage implements IRangedAttackMob {
  ...
  public EntityOrochimaru(World par1World) {
    super(par1World);
  ...
  public boolean interact(EntityPlayer p_70085_1_) {
    if (!this.worldObj.isRemote && p_70085_1_ != null && p_70085_1_.getHeldItem() != null) {
      ...
      if (itemstack.getItem() == AlcoholMod.SealedSageScroll) {
        if (this.Talkative) {
          if (this.beaten != null && this.SageTraining)
            if (this.beaten.toString().equals(p_70085_1_.toString())) {
              p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rХорошо, очень хорошо!!"));
              p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rЯ обновлю местоположение вашего свитка"));
              p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rВ соответствии с пещера Рьючи!"));
            } else {
              p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rТы не доказал мне свою ценность!"));
            }  
        } else {
          this.SageTraining = true;
          p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rО, свиток мудреца..!"));
          p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rХммм, я сниму его, если сможешь."));
          p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rдоказать мне, что ты достоин его!"));
          setHealth(1000.0F);
        } 
      } else if (this.Talkative) {
        if (this.beaten != null)
          if (this.beaten.toString().equals(p_70085_1_.toString())) {
            if (props.getCurseSize() != 2) {
              if (props.getCurseSeal() == 0) {
                if (!this.worldObj.isRemote) {
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rххммм... Вы показываете потенциал!"));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rЯ думаю, что могу сделать тебя еще более"));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rгрозным шиноби! Вот, возьми кусочек."));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rмоей силы!"));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rНайди меня, чтобы добиться еще более мощной силы ха-ха-ха!!"));
                  int randomNumber = this.rand.nextInt(6);
                  props.setCurseSize(1);
                  PacketDispatcher.sendTo((IMessage)new SyncCurseSizeMessage(p_70085_1_), (EntityPlayerMP)p_70085_1_);
                  if (randomNumber == 0) {
                    props.setCurseSeal(1);
                    PacketDispatcher.sendTo((IMessage)new SyncCurseSealMessage(p_70085_1_), (EntityPlayerMP)p_70085_1_);
                    p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Ты чувствуешь слабость, когда Орочимару кусает твою шею!"));
                    p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Кажется, он разблокировал определенную силу!"));
                    p_70085_1_.addPotionEffect(new PotionEffect(Potion.confusion.id, 1000, 50));
                    p_70085_1_.addPotionEffect(new PotionEffect(Potion.weakness.id, 1000, 50));
              ...
              } else if (props.getCurseSeal() > 0) {
                if (!this.Follow) {
                  if (!this.worldObj.isRemote) {
                    p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rТак ты хочешь присоединиться ко мне?"));
                    p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rЯ знал, что ты придешь в конце концов!"));
                    p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rХа-Ха-Ха-Ха"));
                    p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rПоговори со мной еще раз, чтобы все уладить!"));
                    this.Follow = true;
                  } 
                } else if (this.Follow && !this.worldObj.isRemote) {
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rХорошо! Отныне ты Саннин"));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rИ житель скрытого звука шиноби!"));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rХа-Ха-Ха-Ха! Здесь, я дарую вам полную"));
                  p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rТрансформацию Проклятой Печати!"));
                  props.setRank(5);
                  PacketDispatcher.sendTo((IMessage)new SyncRankMessage(p_70085_1_), (EntityPlayerMP)p_70085_1_);
                  props.setAffiliation(13);
                  PacketDispatcher.sendTo((IMessage)new SyncAffiliationMessage(p_70085_1_), (EntityPlayerMP)p_70085_1_);
                  props.setCurseSize(2);
                  PacketDispatcher.sendTo((IMessage)new SyncCurseSizeMessage(p_70085_1_), (EntityPlayerMP)p_70085_1_);
                  p_70085_1_.addPotionEffect(new PotionEffect(Potion.confusion.id, 1000, 50));
                  p_70085_1_.addPotionEffect(new PotionEffect(Potion.weakness.id, 1000, 50));
                  dropItem(AlcoholMod.chestplateSound, 1);
                  dropItem(AlcoholMod.bootsSound, 1);
                  dropItem(AlcoholMod.helmetSound, 1);
                  dropItem(AlcoholMod.legsSound, 1);
                } 
              } 
            } else if (props.getAffiliation() != 13 && props.getCurseSeal() != 0 && props.getCurseSize() == 2) {
              p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rПочему вы покинули мою деревню?!"));
            } else if (props.getAffiliation() == 13 && props.getCurseSeal() != 0 && props.getCurseSize() == 2) {
              p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rМой дорогой шиноби, что-то случилось? Ха-Ха-Ха-Ха"));
            } 
          } else if (!this.worldObj.isRemote) {
            p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rЯ не заинтересован в тебе!"));
          }  
      } else if (props.getAffiliation() == 13 && props.getCurseSeal() != 0 && props.getCurseSize() == 2) {
        p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rМой дорогой шиноби, что-то случилось? Ха-Ха-Ха-Ха"));
      } else {
        p_70085_1_.addChatMessage((IChatComponent)new ChatComponentText("§5Орочимару: §rПопробуй меня, если вы думаете, что сможете заинтересовать меня!"));
      } 
    } 
