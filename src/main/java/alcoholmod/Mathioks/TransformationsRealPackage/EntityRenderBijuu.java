...
public class EntityRenderBijuu extends EntityRenderer {
  ...
  public EntityRenderBijuu(Minecraft mc) {
    super(mc, mc.getResourceManager());
    this.mc = mc;
  }
  
  public EntityRenderBijuu(Minecraft mc, float offset) {
    super(mc, mc.getResourceManager());
    this.mc = mc;
    this.offsetY = offset;
  }
  ...
  public void updateCameraAndRender(float partialTick) {
    if (this.mc.thePlayer != null && !this.mc.thePlayer.isPlayerSleeping()) {
      if ((!this.mc.thePlayer.isSneaking() && !this.mc.thePlayer.isSprinting()) || this.offsetY != 2.0F) {
