...
public class Easter2018Genjutsu extends JutsuItem {
  @SideOnly(Side.CLIENT)
  public EnumRarity getRarity(ItemStack par1ItemStack) {
    return EnumRarity.epic;
  }
  
  public Easter2018Genjutsu() {
    this.maxStackSize = 1;
  }
  ...
  public ItemStack onItemRightClick(ItemStack Par1ItemStack, World Par2World, EntityPlayer Par3EntityPlayer) {
    if (!Par2World.isRemote) {
      ...
      if (props.getCurrentMana() >= 50) {
        if (props.getGenjutsu() >= 30) {
          if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
            if (Par1ItemStack.stackTagCompound.getInteger("Obtained") == 1) {
              Par2World.spawnEntityInWorld((Entity)new Easter2018EggsorcismEntity(Par2World, (EntityLivingBase)Par3EntityPlayer));
              props.consumeMana(50);
              Par3EntityPlayer.addChatComponentMessage((IChatComponent)new ChatComponentText("Genjutsu: Eggsorcism!"));
            } else {
  ...
  public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    ...
    par3List.add(EnumChatFormatting.DARK_AQUA + "A Genjutsu used by Kurousagi.");
    par3List.add(EnumChatFormatting.DARK_AQUA + "Create an easter egg");
    par3List.add(EnumChatFormatting.DARK_AQUA + "that transfers any negative");
    par3List.add(EnumChatFormatting.DARK_AQUA + "potion effects from the user to");
    par3List.add(EnumChatFormatting.DARK_AQUA + "the opponent.");
    par3List.add(EnumChatFormatting.GOLD + "Chakra Cost: 50");
    par3List.add(EnumChatFormatting.GOLD + "Genjutsu needed: 30");
  }
}
