...
public class LeafWhirlwind extends JutsuItem {
            ...
            for (int k2 = 0; k2 < list.size(); k2++) {
              if (!par2World.isRemote) {
                ...
                if (entity instanceof net.minecraft.entity.EntityLiving && entity instanceof net.minecraft.entity.EntityLiving) {
                  entity.attackEntityFrom(DamageSource.causePlayerDamage(player), 4.0F + (int)props.getTaijutsu());
                  if (d0 <= 10.0D) {
  ...
  public ItemStack onItemRightClick(ItemStack Par1ItemStack, World Par2World, EntityPlayer Par3EntityPlayer) {
    int counter = Par1ItemStack.stackTagCompound.getInteger("counter");
    if (counter == 0) {
      Par3EntityPlayer.addVelocity(-Math.sin(Math.toRadians(Par3EntityPlayer.rotationYawHead)) * 3.0D, 0.0D, Math.cos(Math.toRadians(Par3EntityPlayer.rotationYawHead)) * 3.0D);
      if (!Par2World.isRemote)
        if (Par1ItemStack.stackTagCompound.getString("Owner").equals(Par3EntityPlayer.getCommandSenderName())) {
          counter = 700;
          Par1ItemStack.stackTagCompound.setInteger("counter", counter);
          Par3EntityPlayer.addChatComponentMessage((IChatComponent)new ChatComponentText("Leaf Whirlwind!"));
        } else {
  ...
  public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
    ...
    par3List.add(EnumChatFormatting.DARK_AQUA + "A high speed series of");
    par3List.add(EnumChatFormatting.DARK_AQUA + "kicks to the opponent");
    par3List.add(EnumChatFormatting.DARK_RED + "Damage: " + (4 + (int)props.getTaijutsu()));
  }
}
